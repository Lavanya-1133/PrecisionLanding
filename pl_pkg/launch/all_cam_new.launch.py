...
Same as all_cam.launch.py, but with manual exposure controls baked into the
v4l2_camera_node so the camera no longer auto-exposes / whitewashes the frame.

Starts:
  - v4l2_camera_node   (raw camera stream, manual exposure)
...
Override camera settings at runtime:
    ros2 launch pl_pkg all_cam_new.launch.py video_device:=/dev/video2

Manual exposure controls applied (tuned to stop overexposure):
    auto_exposure          = 1   (Manual Mode; default 3 = Aperture Priority/auto)
    exposure_time_absolute = 1     (hardware minimum; default 313)
    brightness             = -64   (default 0)
    gamma                  = 20    (default 67)
    white_balance_automatic = False (disable auto WB drift toward white; default True)
"""
...
def generate_launch_description():
    ...
    v4l2_camera_node = Node(
        ...
        parameters=[{
            ...
            # --- Manual exposure controls (stop overexposure/whitewashing) ---
            'auto_exposure': 1,                  # 1 = Manual Mode (default 3 = auto)
            'exposure_time_absolute': 1,         # hardware minimum (default 313)
            'brightness': -10,                   # default 0
            'gamma': 20,                         # default 67
            'white_balance_automatic': False,    # stop auto WB drifting toward white (default True)
        }],
    )
