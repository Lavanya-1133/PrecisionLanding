...
TICKS_MODE_SWITCH   = 40    # send offboard switch after 2s of heartbeat
...
class OffboardSetup(Node):
    ...
    def __init__(self):
        ...
        self._nav_state    = 0
        self._ticks_arm         = TICKS_ARM
        self._ticks_mode_switch = TICKS_MODE_SWITCH
    ...
    def _publish_hold_setpoint(self):
        """Zero velocity setpoint — drone holds position without commanding movement."""
    ...
    def _loop(self):
        ...
        if self._ticks == self._ticks_arm:
            self._send_arm()
            self._arm_sent = True

        # ── Step 2: switch mode after arm ticks ──────────────────
        if self._ticks == self._ticks_mode_switch:
