...
DESCEND_FAST_ALT    = -2.0    # m NED — altitude to switch to slow descent
LAND_ALT            = -0.5    # m NED — trigger LAND at 0.2m above ground
...
DESCENT_FAST_VEL    =  0.2    # m/s downward during fast descent
DESCENT_SLOW_VEL    =  0.1    # m/s downward during slow descent — keep soft near ground
...
class PrecisionLandController(Node):
    ...
    def __init__(self):
        ...
        self._pre_offboard_ticks = 0 #setpoints ticks before requesting offboard

        # Drone position (NED)
        self.pos_x = 0.0
        self.pos_y = 0.0
        self.pos_z = 0.0
        self._takeoff_start_z = None   # z at moment AWAIT_TAKEOFF begins (baseline)
    ...
    def _send_takeoff_command(self, altitude_m: float = 1.5):
        """
        Send VEHICLE_CMD_NAV_TAKEOFF to PX4.
        Drone must be armed; PX4 transitions to Auto.Takeoff internally.
        altitude_m is relative to home position (positive up).
        """
        msg = VehicleCommand()
        msg.command          = VehicleCommand.VEHICLE_CMD_NAV_TAKEOFF
        msg.param1           = float('nan')   # min pitch (ignored for multirotor)
        msg.param4           = float('nan')   # yaw (keep current)
        msg.param5           = float('nan')   # latitude  (use home)
        msg.param6           = float('nan')   # longitude (use home)
        msg.param7           = float(altitude_m)   # takeoff altitude, metres AGL
        msg.target_system    = 1
        msg.target_component = 1
        msg.source_system    = 1
        msg.source_component = 1
        ...
        msg.timestamp        = int(self.get_clock().now().nanoseconds / 1000)
        self.cmd_pub.publish(msg)
        self.get_logger().info(f'Sent VEHICLE_CMD_NAV_TAKEOFF to {altitude_m:.1f} m AGL')
    ...
    def _loop(self):
        ...
        if self.nav_state != NAV_STATE_OFFBOARD:
            if self.state not in (State.IDLE, State.AWAIT_TAKEOFF, State.PRE_OFFBOARD, State.ABORT, State.DONE):
        ...
        offboard_states = (State.PRE_OFFBOARD, State.TAKEOFF, State.SEARCH, State.ALIGN, State.DESCEND_FAST, State.DESCEND_SLOW, State.RECOVERY, State.LAND)
        ...
        if self.state == State.IDLE:
            if self.arming_state != ARMING_STATE_ARMED:
                ...
                return
            self.get_logger().info('Armed - sending takeoff cmd')
            self._transition(State.AWAIT_TAKEOFF)

        elif self.state == State.AWAIT_TAKEOFF:
            if not self._takeoff_cmd_sent:
                self._takeoff_start_z = self.pos_z       # baseline — captured once, at ground
                self._send_takeoff_command(altitude_m=1.5)
                self._takeoff_cmd_sent = True

            climbed = self._takeoff_start_z - self.pos_z   # NED: climbing decreases z
            target_climb = abs(TAKEOFF_ALT)                # 1.5 m

            self.get_logger().info(
                f'Climbed {climbed:.2f} m / {target_climb:.2f} m target', throttle_duration_sec=1
            )

            if climbed >= target_climb - 0.15:
                self.get_logger().info(f'Climbed {climbed:.2f} m — reached target altitude')
                self._pre_offboard_ticks = 0
                self._transition(State.PRE_OFFBOARD)
                
        # ── PRE_OFFBOARD — stream idle setpoints, then request mode ─
        elif self.state == State.PRE_OFFBOARD:
            # Publish idle position setpoint at current location every tick.
            # PX4 needs ≥ 2 Hz (we're at 20 Hz) of setpoints before it will
            # accept the mode switch.  Wait 20 ticks (~1 s) before asking.
            self._publish_position_setpoint(self.pos_x, self.pos_y, self.pos_z)
            self._pre_offboard_ticks += 1

            if self._pre_offboard_ticks == 20:
                self.get_logger().info(
                    'Setpoint stream established — requesting OFFBOARD mode')
            ...
            if self.nav_state == NAV_STATE_OFFBOARD:
                ...
                self.get_logger().info(
                    f'OFFBOARD accepted at z={self.pos_z:.2f} m — entering SEARCH')
                self._transition(State.SEARCH)
            elif self._pre_offboard_ticks > 60:
                # 3 s with no acceptance — re-send the mode command and keep trying
                self.get_logger().warn(
                    f'OFFBOARD not accepted after {self._pre_offboard_ticks} ticks '
                    f'— re-sending mode command', throttle_duration_sec=2)
                self._send_offboard_mode_command()
                self._pre_offboard_ticks = 40   # back off slightly, don't spam
