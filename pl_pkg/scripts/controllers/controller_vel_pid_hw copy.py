...
class PrecisionLandController(Node):
    ...
    def _loop(self):
        ...
        if self.state == State.IDLE:
            # PX4 grants OFFBOARD only when it sees a valid setpoint stream
            # ALONGSIDE the OffboardControlMode heartbeat (STEP 1). Without
            # this, the mode switch is rejected from every source (our command,
            # QGC, RC) with "no offboard signals". Hold current position while
            # we wait so the offboard pair is complete before the switch.
            ...
            # Retry the OFFBOARD switch periodically rather than once: a single
            # DO_SET_MODE can be missed, and it won't be re-sent otherwise.
            # First attempt after 10 ticks of heartbeat+setpoint (~0.5s), then
            # every 20 ticks (~1s) until PX4 actually reports OFFBOARD.
            if self.offboard_ticks == 10 or (self.offboard_ticks > 10 and self.offboard_ticks % 20 == 0):
